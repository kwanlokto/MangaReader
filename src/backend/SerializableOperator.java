...
public class SerializableOperator {
     ...
     */
    public static HashMap<?, ?> readFile(String pathName) {
        try {
            InputStream file = new FileInputStream(pathName);
            InputStream buffer = new BufferedInputStream(file);
            ObjectInput input = new ObjectInputStream(buffer);
            HashMap<Folder, ?> deserialize_data = (HashMap<Folder, ?>) input.readObject();
            input.close();

            for (Folder key : deserialize_data.keySet()) {
                File tempFile = new File(key.getFolder());
                if (!tempFile.exists()) {
                    deserialize_data.remove(key);
                }
            }
            ...
            return new HashMap<Folder, Integer>();
        }
    }
     ...
     */
    public static void saveFile(String pathName, HashMap<?, ?> objectToSerialize) {
        try {
